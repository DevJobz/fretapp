...
document.addEventListener('DOMContentLoaded', function () {
    // Carregar instituições e empresas do localStorage
    const instituicoes = JSON.parse(localStorage.getItem('instituicoes')) || [];
    const empresas = JSON.parse(localStorage.getItem('empresas')) || [];

    // Formulário de Inserir Instituição
    document
        .getElementById('formInstituicao')
        .addEventListener('submit', function (e) {
            e.preventDefault();

            const nomeFantasia = document.getElementById(
                'nomeFantasiaInstituicao'
            ).value;
            const cnpj = document.getElementById('cnpjInstituicao').value;
            const cep = document.getElementById('cepInstituicao').value;
            const rua = document.getElementById('ruaInstituicao').value;
            const numero = document.getElementById('numeroInstituicao').value;
            const complemento = document.getElementById(
                'complementoInstituicao'
            ).value;
            const bairro = document.getElementById('bairroInstituicao').value;
            const uf = document.getElementById('ufInstituicao').value;
            const cidade = document.getElementById('cidadeInstituicao').value;

            // Verificar duplicidade de CNPJ
            const cnpjExistente = instituicoes.find(
                (instituicao) => instituicao.cnpj === cnpj
            );
            if (cnpjExistente) {
    ...
    document
        ...
        .addEventListener('submit', function (e) {
            ...
            const rua = document.getElementById('ruaEmpresa').value;
